...
int energetic_bonsai(char *filename="../wcsim.root", bool verbose=false) {
	...
	for (int ev=0; ev < tree->GetEntries(); ev++) {
		...
		for (int index = 0 ; index < event->GetNumberOfEvents(); index++) {
			...
			std::vector<float> PMTX (ncherenkovdigihits,0);
			std::vector<float> PMTY (ncherenkovdigihits,0);
			std::vector<float> PMTZ (ncherenkovdigihits,0);
			...
			for (i=0; i<ncherenkovdigihits; i++) {
				...
				pmt = geo->GetPMT(cherenkovdigihit->GetTubeId());
			...
			j=0;
			for (i=0; i<ncherenkovdigihits; i++) { // loop over tCorrected, which is in the same order as the vectors PMTX/Y/Z and bsCAB
